...
x_* dataframes hold the images, and y_* dataframes hold the classification labels (model output).
'''
(x_train, y_train), (x_test, y_test) = keras.datasets.mnist.load_data()
logger.info(f'Loaded the mnist dataset to training and test dataframes.')
...
logger.info(f'Normalized data to scale [0,1].')
...
logger.info(f'Expanded data shape to add color channel.')

'''
Check the shape of the dataframes and log the information.
'''
logger.info(f'Train dataframe shape (images, pwidth, pheight, channel): {x_train.shape}')
logger.info(f'Test dataframe shape (images, pwidth, pheight, channel): {x_test.shape}')

'''
The num_classes parameter indicates how many unique labels the model will assign (we have 10 possible digits to match).
'''
num_classes: int=10
logger.info(f'Number of classifications (unique labels) for the model to apply: {num_classes}')

'''
Implement a prepared, sequential convolutional neural network model (...yikes! I had to look that up!).
'''
